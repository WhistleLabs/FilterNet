...
from filternet.datasets import har as ds
...
def test_get_or_make_dfs(dfs_dict):
    assert dfs_dict["df_train"].shape == (403712, 12)
    assert dfs_dict["df_val"].shape == (66816, 12)
    assert dfs_dict["df_test"].shape == (188672, 12)

    for df in [dfs_dict["df_train"], dfs_dict["df_val"], dfs_dict["df_test"]]:
        assert df.isna().sum().sum() == 0

    assert dfs_dict["s_labels"].shape == (6,)
    assert dfs_dict["df_cols"].shape == (12, 3)
...
def test_urls():
    assert ds.DATASET_FILE == "UCI%20HAR%20Dataset.zip"
    assert ds.DATASET_SUBDIR == "UCI_HAR_Dataset"
